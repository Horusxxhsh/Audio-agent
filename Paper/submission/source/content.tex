...
\caption{Overall framework of the retrieval-grounded guitar-effect control system. The figure summarizes the modular flow from query inputs to asynchronous retrieval, validity-checked parameter update, and executable effect-chain output. It is a framework illustration rather than an experimental result figure.}
\label{fig:framework}
\end{figure*}
...
\label{sec:repro}
The supplementary package contains preprocessing details, embedding computation steps, retrieval-index construction, prompt templates, evaluation scripts, and split specifications for all reported tables. The artifact is anonymized for review and makes each reported table traceable to a script-level evidence path.

\paragraph{Baselines.}
We compare our proposed \textbf{TRR} (Section \ref{ssec:rag}) against locally auditable retrieval baselines. \textbf{Wav2Vec} uses mean-pooled Wav2Vec2 activations. \textbf{FeatureNN} uses the released benchmark feature vectors and should therefore be interpreted as a fixed dense-retrieval reference. \textbf{CLAP} uses the stored CLAP vectors available in the local JSON artifact and is reported with its actual coverage. \textbf{PaSST} is regenerated in the E9 derived artifact with full top-1 retrieval coverage and serves as the full-coverage modern audio-encoder baseline. PANNs remains excluded from unified tables due to model weight download constraints; we do not claim ``best full-coverage'' unconditionally.
...
The paper-facing evidence separates quantitative artifacts from illustrations. Protocol-A/B metrics, robustness curves, and EPR sensitivity are tied to exported CSV/JSON artifacts with fixed split files and coverage accounting; mechanism plots, error diagnostics, listening summaries, and framework diagrams are used only as diagnostics or non-result illustrations. The detailed provenance audit is retained in the source package rather than expanded in the main text.

Table~\ref{tab:main_results} reports the unified comparison. PaSST is regenerated in the E9 derived artifact with full top-1 retrieval coverage and is included in both the retrieval and projection comparisons. PANNs remains excluded due to model weight download constraints; we therefore do not claim a fair PANNs EPR-K5 baseline.
...
The diagnostics are consistent with the bounded mechanism interpretation used in the supplementary material. Same-backbone Gram aggregation improves over mean pooling in the archived and local mechanism checks, but the gain is partly entangled with mid-level layer choice and projection dimension. Controlled ablation shows that Gram statistics improve normalized L2 by 0.0125 at layer~5 alone (Wilcoxon $p=0.586$) but only 0.0007 at multi-layer $\{4,5,6\}$ ($p=0.767$); neither paired test reaches significance, yet the effect-size difference between layer-5 and multi-layer confirms that the advantage is coupled with layer selection. For this reason, the paper claims TRR as a useful texture-aware retrieval prior, not as a globally optimized audio representation family. The per-parameter view also makes clear that a lower aggregate normalized distance does not eliminate localized weaknesses: switch-like structures, sparse effect leaves, and parameters with nonlinear perceptual scales still require separate validity and perceptual audits.
...
Second, a 26-participant multiple-stimulus listening study provides perceptual context. Figure~\ref{fig:listening_trials2_5} summarizes the manual-baseline comparison trials; the remaining listening distributions are reported in the supplementary material. The observed distributions support a cautious interpretation: the TRR-based system is competitive in the tested listening setup, but the study is not a standard MUSHRA protocol and does not validate the objective metrics as human-rating proxies. Because it lacks a standard low-quality anchor, device metadata, participant-expertise labels, and a documented manual-tuning time budget, we treat it as supporting context rather than a primary validation of the retrieval or projection protocols. A metric--perception alignment diagnostic is also negative under the available alias mapping. Together, these results reinforce that the main claim is parameter-space transferability under audited retrieval protocols, not a validated predictor of human listening scores.
...
The evidence is bounded in four ways. First, the objective benchmark is a 204-query, guitar-only split with one effect-chain topology; resolved-audio grouping removes exact cross-split reuse, but near-duplicate risk, corpus density, and cross-domain generalization remain open. Second, normalized parameter metrics are engineering proxies rather than perceptual scales, and the listening diagnostic is not a standard MUSHRA-style, query-aligned user study; we therefore do not claim that parameter-space gains directly imply perceived quality gains or workflow improvement. Third, the baseline claim is conditional on the evaluated methods: CLAP, PaSST, and same-backbone ablations strengthen the comparison, whereas PANNs remains excluded because its model weights were not available in the artifact environment. Fourth, EPR-K5 is a projected blend of retrieved exemplars rather than pure retrieval, so its range/switch repair step is part of the deployed method. Personalization, memory, adaptive fusion, uncached live-reference latency, and out-of-distribution database expansion are outside the validated scope.
