...
Table~\ref{tab:main_results} reports the unified comparison. PaSST vectors were regenerated in the E9 derived artifact and pass the 204-query/1,063-candidate coverage gate, so PaSST is included in both Protocol-A and Protocol-B. PANNs remains excluded due to model weight download constraints; we therefore do not claim a fair PANNs EPR-K5 baseline.
...
\item \textbf{Tolerance sensitivity.} Analysis across tolerances $\tau \in \{0.05, 0.10, 0.15, 0.20\}$ shows TRR maintains its advantage over all baselines at every tolerance value (TRR Acc@$\tau$: 0.172, 0.461, 0.588, 0.662), confirming that results are not specific to a single tolerance choice.
\item \textbf{Multiple-comparison correction.} All pairwise Wilcoxon tests remain significant after Holm-Bonferroni correction (largest corrected $p = 1.63 \times 10^{-5} < 0.05$), confirming that the statistical claims are robust to multiple-comparison correction.
\end{itemize}
