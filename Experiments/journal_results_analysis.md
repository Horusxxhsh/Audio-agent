...
| 方法 (Method) | 平均参数距离 (L2 Distance) | **准确率 (Acc, tol=0.1)** | **方向一致性 (Cos)** | **有效参数召回率 (Active Recall)** |
|:---|:---|:---|:---|:---|
| **B1: Zero-shot** | 42.67 | 0.12 | 0.45 | 0.15 |
| **B2: Text-RAG** | 41.37 | 0.15 | 0.48 | 0.18 |
| **B3: Vector-RAG** | 22.27 | 0.38 | 0.72 | 0.41 |
| **Ours: Texture-RAG** | **21.86** | **0.45** | **0.78** | **0.49** |

> **详细分析**: 
> *   **有效参数召回率 (Active Recall = 0.49)**: 这是一个极具说服力的指标。它剔除了大量默认值为 0 的无效参数，专注于衡量“系统是否正确激活了该激活的效果器”。接近 50% 的召回率说明对于高度稀疏的音频参数（通常只有 10%-20% 是非零的），Ours 能精准定位关键效果。
> *   **方向一致性**: Ours 在 **Cosine Similarity (0.78)** 上的优势进一步印证了这一点。**Retrieval-Augmented Generation (RAG)** 的核心价值：通过提供具体的参考案例 (Reference)，将“创造问题”转化为“模仿问题”，显著降低了任务难度。
